...
/*
//////this did not work because a group was the child of the clippath
function addPathWithOrigin(parentTag, name, origin) {
  var transGroup = parentTag.append('g')
                            .attr('transform', 'translate(' + origin.x + ',' + origin.y + ')');

  transGroup.selectAll('path').data([arcObj])
    .enter().append('path')
          .attr('id', name)
          .attr('class', 'geom')
          .attr('d', function(d) {return arc(d);});
...
var toReturn = parentTag.append('clipPath')
                      .attr('id', name);
    addPathWithOrigin(toReturn, 'clippersPath', {x: arcOrigin.x, y: arcOrigin.y});                   
}
